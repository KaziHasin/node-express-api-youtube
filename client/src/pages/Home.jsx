    ...
    const showText = (text, id) => {
        ...
        if (text.length > maxLength) {
            if (readMoreStates[id]) {
                return (
                    <>{text}<span className="text-blue-600 cursor-pointer" onClick={toggleReadMore(id)}>Read Less</span></>
                )
            } else {
                return (
                    <>
                        {text.substring(0, maxLength)}... <span className="text-blue-600 cursor-pointer" onClick={toggleReadMore(id)}>Read More</span>
                    </>
                )
            }
                            ...
                            products.map(product => (
                                <tr key={product._id}>
                                    ...
                                    <td className='border-b border-gray-200 py-2'>
                                        {showText(product.description, product._id)}
                                    </td>
                                    <td className='border-b border-gray-200 py-2'>
